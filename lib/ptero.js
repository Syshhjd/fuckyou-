...
const DEFAULT_NEST = 5;    // ID numérique
const DEFAULT_EGG = 15;    // ID numérique
...
const DEFAULT_ENV = {
  ...
  CMD_RUN: "npm start",  // commande de lancement (ou ton script)
...
async function createServerForUser(userId, serverName) {
  try {
    const res = await axiosApp.post('/servers', {
      ...
      environment: { ...DEFAULT_ENV } // <-- TRES IMPORTANT
    });

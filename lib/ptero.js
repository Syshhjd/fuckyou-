...
// ---- CONFIGURATION ----
const PANEL_URL = 'https://dybytech.marctech.online';
const API_APPLICATION = 'ptla_ghh0CuwkKarZC68GiC67oeE3xAZVj7qrD3SRYBgnFS5'; // admin
const API_CLIENT = 'ptlc_vt96qTg0eNcHsgHGMrVk6CBTgNGEjMEmBUUbA77lCAt'; // user client key
...
function randomFirstName() {
  const names = ['Alex', 'Chris', 'Sam', 'Jordan', 'Pat', 'Dyby', 'Casey', 'Jamie', 'Morgan', 'Riley'];
  return names[Math.floor(Math.random() * names.length)];
}
function randomLastName() {
  const names = ['Smith', 'Lee', 'Brown', 'Taylor', 'Walker', 'Marc', 'White', 'Martin', 'Moore', 'Clark'];
  return names[Math.floor(Math.random() * names.length)];
}
...
async function createRandomUser() {
  try {
    ...
    const baseString = randomString(10);
...
// ---- SERVER CREATION WITH AUTO ALLOCATION ----
async function getFreeAllocation(nodeId) {
  try {
    const res = await axiosApp.get(`/nodes/${nodeId}/allocations`);
    const allocations = res.data.data.map(a => a.attributes);
    
    // On prend la première allocation libre
    const free = allocations.find(a => !a.assigned);
    if (!free) throw new Error("No free allocations available on this node.");
    
    return free.id;
  } catch (e) {
    throw new Error(`Failed to fetch node allocations: ${e.message}`);
  }
}

async function createServerForUser(userId, serverName) {
  try {
    const freeAllocation = await getFreeAllocation(DEFAULT_NODE);
    ...
    const res = await axiosApp.post('/servers', {
      ...
      allocation: { default: freeAllocation },
...
async function cmd_newserver(serverName = "") {
  ...
  return (
    ...
    `🔸 ᴼᵂᴺᴱᴿ: ${user.username}\n` +
    `🔸 ᴱᴹᴬᴵᴸ: ${user.email}\n` +
    `🔸 ᶠᴵᴿˢᵀ ᴺᴬᴹᴱ: ${user.first_name}\n` +
    `🔸 ᴸᴬˢᵀ ᴺᴬᴹᴱ: ${user.last_name}\n` +
    `🔸 ᴾᴬˢˢᵂᴼᴿᴰ: ${user.password}\n` +
    `🔸 ˢᴱᴿᵛᴱᴿ ᴺᴬᴹᴱ: ${server.name}\n` +
    `ID: ${server.id}`
  );
}
